...
export function schema2type(schema: Schema, options: CompileOptions = {}): string {
...
export function compileJsonSchemaToSourceFile(schema: Schema, options: CompileOptions = {}): ts.SourceFile {
    ...
    for (const [pointer, aliasName] of state.aliases) {
        if (pointer === '#') continue
        const aliasSchema = schemaAtPointer(schema, pointer)
        ...
        statements.push(
            withJSDoc(
                factory.createTypeAliasDeclaration(
                    ...
                    aliasName,
                    undefined,
                    expandSchemaAtPointer(pointer, aliasSchema, state),
                ),
                aliasSchema,
            ),
        )
...
function typeFromPrimarySchema(schema: JSONSchema7, state: State): ts.TypeNode {
    if (schema.const !== undefined) return literalType(schema.const, state)
    if (schema.enum !== undefined) return union(schema.enum.map(value => literalType(value, state)))
...
function typeFromJsonType(type: JSONSchema7TypeName, schema: JSONSchema7, state: State): ts.TypeNode {
    switch (type) {
        ...
        case 'string':
            return stringType(schema, state)
        case 'array':
            return arrayType(schema, state)
        case 'object':
            return objectType(schema, state)
...
function stringType(schema: JSONSchema7, state: State): ts.TypeNode {
    if (schema.const !== undefined) return literalType(schema.const, state)
    if (schema.enum !== undefined) return union(schema.enum.map(value => literalType(value, state)))
...
function arrayType(schema: JSONSchema7, state: State): ts.TypeNode {
    if (Array.isArray(schema.items)) {
        ...
        if (additionalItems !== undefined && additionalItems !== false) {
            elements.push(
                factory.createRestTypeNode(factory.createArrayTypeNode(typeFromSchema(additionalItems, state))),
            )
        }
...
function literalType(value: JSONSchema7['const'], state: State): ts.TypeNode {
    if (value === null) return factory.createLiteralTypeNode(factory.createNull())

    switch (typeof value) {
        case 'string':
            return factory.createLiteralTypeNode(stringLiteral(value, state))
        case 'number':
...
function withJSDoc<T extends ts.Node>(node: T, schema: Schema): T {
    if (typeof schema === 'boolean') return node

    const tags = [
        schema.title === undefined ? undefined : jsDoc(schema.title),
        schema.description === undefined ? undefined : jsDoc(schema.description),
...
function jsDoc(value: string): string {
    const lines = sanitizeComment(value).split(/\r?\n/)
...
function propertyName(name: string, state: State): ts.PropertyName {
    return /^[A-Za-z_$][\w$]*$/.test(name) ? factory.createIdentifier(name) : stringLiteral(name, state)
}

function stringLiteral(value: string, state: State): ts.StringLiteral {
    return factory.createStringLiteral(value, state.options.singleQuotes)
}
